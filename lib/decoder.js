        ...
        for (let gy = 0; gy < this._grid; ++gy) {
            for (let gx = 0; gx < this._grid; ++gx) {
                for (let bi = 0; bi < this._nBoxes; ++bi) {
                    ...
                    const rec = (() => {
                        const x0 = (gx + sigmoid(loc[1])) * block.w;
                        const y0 = (gy + sigmoid(loc[0])) * block.h;
                        const x1 = x0 + Math.exp(loc[3]) * this._anchors[bi][1] * block.w;
                        const y1 = y0 + Math.exp(loc[2]) * this._anchors[bi][0] * block.h;
                        return new Rectangle(...[x0, y0, x1, y1].map(Math.floor));
                    })();

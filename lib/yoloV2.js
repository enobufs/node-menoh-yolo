...
class YoloV2 {
    ...
    build(onnxPath) {
        ...
        .then((builder) => {
            ...
            builder.addInput(this._config.inputName, [
                this._config.batchSize, // 1 by default
    ...
    // @param {ndarray) images Input images in the form of NCHW.
    ...
    decode(result, option) {
        const boxes = this._decoder.decode( result,
                                            option.scoreThresh,
                                            option.overlapThresh,
                                            option.originalShape);
